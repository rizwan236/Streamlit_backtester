...
def calculate_trades(df, buy_rsi, buy_cci, sell_rsi, sell_cci):
    ...
    for i in range(0, len(df)):
        ...
        if not in_position and  SMAAD2 * 1.0 < AD and ADX3 < ADX > 25 and PLUS_DI > PLUS_DI3 and (PLUS_DI- MINUS_DI) > 5 and volume_val >= 500 and ( MOMScore > -1.5 or weighted_excessMR > -0.4)  and (ST == 1 and EMAMRP24 < MRP and EMAOBV * 1.0 < OBV and close_val > SMAClose10 > SMAClose30 > SMAClose40 and SMAClose40_21 < SMAClose40 > sma_based_sma200 and RSI_e > buy_rsi and  RSI_min > 40 and RSI_max > 58 and close_val > wkH52 * .75 and close_val > wkL52 * 1.35):    
...
def create_chart(df, buy_rsi, buy_cci, sell_rsi, sell_cci, symbol,):    
    ...
    ax1.plot(df.index, df['SMAClose10'], color='darkgreen', linewidth=2, label='Close Price')
    ax1.plot(df.index, df['SMAClose30'], color='green', linewidth=2, label='Close Price')
    ax1.plot(df.index, df['SMAClose40'], color='lightgreen', linewidth=2, label='Close Price')
    ax1.plot(df.index, df['sma_based_sma200'], color='limegreen', linewidth=2, label='Close Price')
