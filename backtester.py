...
try:
    ...
    filtered = latest_data[(latest_data["Volume_50SMA"] > 10000) & (
        latest_data["RSI_e"] > 45) & (latest_data["Score"] > 2.3)]    
    
    # 3. Select Symbol and Score, sort descending, take top 25, and rename columns
    top_25 = filtered[["Symbol", "Score"]].sort_values("Score", ascending=False).head(90).rename(
...
def calculate_trades(df, buy_rsi, buy_cci, sell_rsi, sell_cci):
    ...
    for i in range(0, len(df)):
        ...
        #elif in_position and RSI_e < 50:# and cci_val < sell_cci :
...
with tab3:
   ...
   #           'Score','ZScore3m','Fin_score', 'SMA_200C', 'RSI_e','avgRSI']].copy()
    ...
    if 'latest_data' in locals() and latest_data is not None:
        st.data_editor(
            latest_data[['Symbol', 'Date', 'Close', 'High', 'Low', 'Open', 'Volume', 'Stock_Cumulative_Return', 'MRP','DD_PCT', 'ST', 'OBV', 'weighted_excessMR', 'weighted_MR', 'Score', 'SMA_200C', 'RSI_e']],
            #latest_data,
            use_container_width=True,
            num_rows="fixed",
